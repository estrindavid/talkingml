...
<% if (items.length === 0) { %>
  <section class="archive-empty" aria-labelledby="empty-notes-archive-title">
    <p class="archive-empty-number" aria-hidden="true">00</p>
    <h2 id="empty-notes-archive-title">Oops, there are no notes here yet :/</h2>
    <p>The first note is still being prepared. Recent articles are open on the <a href="/">homepage</a>.</p>
  </section>
<% } else { %>
